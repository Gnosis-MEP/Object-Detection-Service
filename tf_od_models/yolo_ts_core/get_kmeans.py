...
def fix_annotations(annotation_path):
    with open(annotation_path, 'r') as anno:
        with open(f'{annotation_path.replace("_old", "")}', 'w') as fixed:
            result = []
            for line in anno:
                s = line.strip().split(' ')
                path = s[0]
                ...
                for i in range(box_cnt):
                    # image_path x_min y_min x_max y_max class_id  x_min y_min ... class_id
                    x_min, y_min, x_max, y_max, class_id = float(s[i*5+0]), float(s[i*5+1]), float(s[i*5+2]), float(s[i*5+3]), int(s[i*5+4])
                    stringed_values = [str(x) for x in [class_id, x_min, y_min, x_max, y_max]]
                    final_str = ' '.join([final_str] + stringed_values)
                final_str = final_str + '\n'
                fixed.write(final_str)
    return result
...
if __name__ == '__main__':
    ...
    # fix_annotations(annotation_path)
    # print('done')
    # exit()
